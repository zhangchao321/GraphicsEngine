...
namespace
{
	char const* VERT_SHADER_SOURCE_DEPTH("gl-460/fbo-shadow-depth.vert");
	char const* VERT_SHADER_SOURCE_RENDER("gl-460/fbo-shadow-render.vert");
	char const* FRAG_SHADER_SOURCE_RENDER("gl-460/fbo-shadow-render.frag");
...
public:
	sample(int argc, char* argv[]) :
		framework(argc, argv, "gl-460-fbo-shadow", framework::CORE, 4, 0, glm::vec2(0.0f, -glm::pi<float>() * 0.3f))
	{}
...
private:
	bool initProgram()
	{
		...
		{
			compiler Compiler;
			GLuint VertShaderName = Compiler.create(GL_VERTEX_SHADER, getDataDirectory() + VERT_SHADER_SOURCE_RENDER, "--version 460 --profile core");
			GLuint FragShaderName = Compiler.create(GL_FRAGMENT_SHADER, getDataDirectory() + FRAG_SHADER_SOURCE_RENDER, "--version 460 --profile core");
	...
	void renderShadow()
	{
		...
		glPolygonOffsetClamp(0.0f, 1.0f, 0.0f);

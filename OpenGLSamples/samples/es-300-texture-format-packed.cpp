...
namespace
{
	char const* VERT_SHADER_SOURCE("es-300/texture-format-packed.vert");
	char const* FRAG_SHADER_SOURCE("es-300/texture-format-packed.frag");
...
private:
	...
	bool initProgram()
	{
		...
		compiler Compiler;
		GLuint VertShaderName = Compiler.create(GL_VERTEX_SHADER, getDataDirectory() + VERT_SHADER_SOURCE, "--version 300 --profile es");
		GLuint FragShaderName = Compiler.create(GL_FRAGMENT_SHADER, getDataDirectory() + FRAG_SHADER_SOURCE, "--version 300 --profile es");
	...
	bool initTexture()
	{
		...
		GLuint const DataRGBA8 = 0xCC0088FF;
		GLushort const DataBGRA4 = 0x08FC;
		GLuint const DataBGRA8 = 0xCCFF8800;
		...
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
		glTexImage2D(GL_TEXTURE_2D, static_cast<GLint>(0), GL_RGBA8, 1, 1, 0, GL_RGBA, GL_UNSIGNED_BYTE, &DataRGBA8);
		...
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_SWIZZLE_B, GL_RED);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_SWIZZLE_A, GL_ALPHA);
		glTexImage2D(GL_TEXTURE_2D, static_cast<GLint>(0), GL_RGBA8, 1, 1, 0, GL_RGBA, GL_UNSIGNED_BYTE, &DataBGRA8);

...
namespace
{
	char const* VERT_SHADER_SOURCE_TEXTURE("gl-320/fbo-readpixels-sample.vert");
	char const* FRAG_SHADER_SOURCE_TEXTURE("gl-320/fbo-readpixels-sample.frag");
	char const* VERT_SHADER_SOURCE_SPLASH("gl-320/fbo-readpixels-blit.vert");
	char const* FRAG_SHADER_SOURCE_SPLASH("gl-320/fbo-readpixels-blit.frag");
...
public:
	sample(int argc, char* argv[]) :
		framework(argc, argv, "gl-320-fbo-readpixels", framework::CORE, 3, 2),
		FramebufferName(0),
		FramebufferScale(2),
		UniformTransform(-1)
	{}
...
private:
	...
	std::vector<glm::uint> ReadBuffer;
	...
	bool begin()
	{
		...
		this->ReadBuffer.resize(640 * 480);
	...
	bool render()
	{
...

		{
			...
			glReadPixels(0, 0, 640, 480, GL_RGBA, GL_UNSIGNED_BYTE, &this->ReadBuffer[0]);
		}
